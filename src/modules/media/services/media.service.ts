...
import { getMediaConfig, uploadLocalFile } from '../helpers';
...
export class MediaService extends BaseService<MediaEntity, MediaRepository> {
    ...
    async upload<E extends ObjectLiteral>({ file, dir, relation, user }: CreateFileOptions<E>) {
        ...
        item.file = uploadLocalFile(uploader, dir);
        if (user) {
        ...
        if (!isNil(relation)) {
            ...
            if (!multi) {
                const oldMedia = !isNil(relationItem[field])
                    ? join(getMediaConfig<string>('upload'), relationItem[field].file)
                    : null;
                ...
                for (const media of medias) {
                    const filePath = join(getMediaConfig<string>('upload'), media.file);
